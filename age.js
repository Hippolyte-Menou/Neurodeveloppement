...
class Age {
  ...
  convert(){
    ...
    this.years_str = this.y > 0 ? `${this.y} an${this.y > 1 ? 's' : ''}` : '';
    this.months_str = this.m > 0 ? `${this.m} mois` : '';
    
    this.and = this.years_str && this.months_str ? 'et' : '';
  ...
  show_slider(){
    ...
    this.x = map(this.months, 0, max_month, 80, width-side_panel);
    this.y = age_slider_y;
    this.w = 60; // Width
    this.h = 30; // Height
